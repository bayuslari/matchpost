...
export function ScoreboardTable({ headerColor, match, profile, displayName, nameDisplayMode, sortedSets }: ScoreboardTableProps) {
  return (
    <>
      ...
      <div className="backdrop-blur-sm rounded-t-xl px-2.5 py-1.5 flex justify-between items-center" style={{ backgroundColor: `${headerColor}E6` }}>
        <span className="text-white text-[10px] font-semibold">
          {match.match_type === 'doubles' ? 'Doubles' : 'Singles'}
        </span>
        <div className="flex gap-0.5">
          {sortedSets.map((_, i) => (
            <span key={i} className="text-white/90 text-[10px] font-semibold w-5 text-center">{i + 1}</span>
          ))}
        </div>
      </div>
      ...
      {match.match_type === 'doubles' ? (
        <div className="bg-white px-2.5 py-1.5 flex items-center gap-1.5 border-b border-gray-100">
          <div className="flex-1 min-w-0 space-y-0.5">
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">
                {profile?.avatar_url ? (
              ...
              <div className="font-semibold text-xs text-gray-900 truncate">{displayName}</div>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">
        ...
        <div className="bg-white px-2.5 py-2 flex items-center gap-1.5 border-b border-gray-100">
      ...
      {/* Opponent Row(s) */}
      {match.match_type === 'doubles' ? (
        <div className="bg-white px-2.5 py-1.5 flex items-center gap-1.5 rounded-b-xl">
          <div className="flex-1 min-w-0 space-y-0.5">
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">
        ...
        <div className="bg-white px-2.5 py-2 flex items-center gap-1.5 rounded-b-xl">
          <div className="w-6 h-6 rounded-full overflow-hidden bg-gray-200 flex-shrink-0 flex items-center justify-center">
            {match.opponent_profile?.avatar_url ? (
              <img src={match.opponent_profile.avatar_url} alt="" className="w-full h-full object-cover" />
            ) : (
      ...
      {/* Match Info Footer */}
      <div className="backdrop-blur-sm rounded-xl mt-1.5 px-2.5 py-1.5 flex justify-between items-center" style={{ backgroundColor: `${headerColor}E6` }}>
